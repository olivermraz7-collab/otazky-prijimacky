import streamlit as stimport jsonimport reimport randomimport osimport hashlibimport secretsfrom datetime import date, timedeltafrom html import escapefrom streamlit_cookies_manager import EncryptedCookieManager
...
st.set_page_config(page_title="Prijímačky",page_icon="🎓",layout="wide",initial_sidebar_state="expanded")
...
APP_NAME = "Prijímačky"APP_SUBTITLE = "príprava na prijímacie skúšky"

DAILY_GOAL = 130RECENT_LIMIT = 8FINAL_MODE_DATE = date(2026, 6, 10)

DATA_DIR = "data"PROGRESS_DIR = os.path.join(DATA_DIR, "progress")USERS_FILE = os.path.join(DATA_DIR, "users.json")

REPORT_FORM_BASE_URL = "https://docs.google.com/forms/d/e/1FAIpQLScVa1VK6mJYX6YRmgcms64AMxaTm5wSDmJF9vnl1M4QzzmCUw/viewform"

FIELDS = {"Všeobecné lekárstvo": {"Biológia": "biologia.json","Chémia": "chemia.json"},"Urgentná medicína": {"Náuka o spoločnosti": "nos.json","Fyzika": "fyzika.json","Biológia": "biologia-urgent.json"}}
...
def inject_css():st.markdown(""" {--bg: #09090b;--bg-soft: #0f172a;--card: rgba(17, 24, 39, 0.88);--card-solid: #111827;--border: rgba(255, 255, 255, 0.08);--text: #f9fafb;--muted: #9ca3af;--primary: #8b5cf6;--primary-2: #3b82f6;--shadow: 0 24px 70px rgba(0, 0, 0, 0.42);--shadow-soft: 0 16px 40px rgba(0, 0, 0, 0.28);}
...
cookies = EncryptedCookieManager(prefix="prep_app_dark_v3/",password="Heslo1234")
...
def ensure_data_dirs():os.makedirs(DATA_DIR, exist_ok=True)os.makedirs(PROGRESS_DIR, exist_ok=True)

ensure_data_dirs()

def read_json_file(path, default):try:if not os.path.exists(path):return default
...
def write_json_file(path, data):tmp_path = path + ".tmp"
...
def normalize_username(username):username = username.strip().lower()username = re.sub(r"[^a-z0-9_.-]", "", username)username = username.strip(".-")return username

def hash_password(password, salt=None):if salt is None:salt = secrets.token_hex(16)
...
def verify_password(password, stored_hash):try:salt, correct_hash = stored_hash.split("$", 1)attempted_hash = hash_password(password, salt).split("$", 1)[1]return secrets.compare_digest(attempted_hash, correct_hash)

except Exception:
    return False

def load_users():data = read_json_file(USERS_FILE, default={"users": {}})
...
def save_users(data):write_json_file(USERS_FILE, data)

def create_user(username, display_name, password):username = normalize_username(username)
...
def authenticate_user(username, password):username = normalize_username(username)users_data = load_users()user = users_data["users"].get(username)
...
def get_logged_user_from_cookie():username = cookies.get("logged_in_user")
...
def login_user(user):st.session_state.authenticated = Truest.session_state.username = user["username"]st.session_state.display_name = user["display_name"]

cookies["logged_in_user"] = user["username"]
cookies.save()

def logout_user():cookies["logged_in_user"] = ""cookies.save()
...
def render_login_screen():st.markdown(f"""{escape(APP_NAME)}Vitaj späť.Prihlás sa a pokračuj v príprave presne tam, kde si skončil.""",unsafe_allow_html=True)
...
if "authenticated" not in st.session_state:cookie_user = get_logged_user_from_cookie()
...
if not st.session_state.authenticated:render_login_screen()st.stop()
...
def get_user_progress_path(username):safe_username = normalize_username(username)return os.path.join(PROGRESS_DIR, f"{safe_username}.json")

def default_user_state():return {"subjects_data": {},"last_settings": {"field_idx": 0,"subj_name": None}}

def load_user_state(username):path = get_user_progress_path(username)user_state = read_json_file(path, default=None)
...
def save_user_state(username, user_state):path = get_user_progress_path(username)write_json_file(path, user_state)

def save_progress():username = st.session_state.username
...
if st.session_state.get("loaded_user") != st.session_state.username:loaded_state = load_user_state(st.session_state.username)
...
def parse_date_safe(value):try:if not value:return None
...
def load_questions(file_path):try:with open(file_path, "r", encoding="utf-8") as f:return json.load(f)

except Exception:
    return []

def get_qid(q):return str(q["id"])

def default_question_progress():return {"status": "NEW","level": 0,"correct_count": 0,"wrong_count": 0,"streak": 0,"first_seen": None,"last_seen": None,"next_review": today_str(),"last_result": None}

def default_daily_stats():return {"answered": 0,"correct": 0,"wrong": 0,"new_seen": 0}

def ensure_subject_state(selected_file, questions):if selected_file not in st.session_state.subjects_data:st.session_state.subjects_data[selected_file] = {"score": 0,"total_count": len(questions),"progress": {},"daily_stats": {},"recent_question_ids": [],"smart_review_version": 1}
...
def get_daily_stats(subject_state):d = today_str()
...
def get_question_progress(subject_state, qid):progress = subject_state["progress"]
...
def count_statuses(subject_state, questions):counts = {"NEW": 0,"RED": 0,"YELLOW": 0,"GREEN": 0,"MASTERED": 0}
...
def dynamic_daily_new_limit(counts):red_count = counts.get("RED", 0)
...
def question_priority(q, subject_state, counts):qid = get_qid(q)p = get_question_progress(subject_state, qid)stats = get_daily_stats(subject_state)
...
def choose_next_question(questions, subject_state):counts = count_statuses(subject_state, questions)recent_ids = subject_state.get("recent_question_ids", [])
...
def get_question_by_id(questions, qid):for q in questions:if get_qid(q) == str(qid):return q

return None

def update_recent_questions(subject_state, qid):recent = subject_state.get("recent_question_ids", [])recent.append(str(qid))recent = recent[-RECENT_LIMIT:]subject_state["recent_question_ids"] = recent

def update_progress_after_answer(subject_state, qid, is_correct):p = get_question_progress(subject_state, qid)stats = get_daily_stats(subject_state)
...
def progress_percent(value, goal):if goal <= 0:return 0.0
...
def status_label(status):labels = {"NEW": "Nová","RED": "Problémová","YELLOW": "Na opakovanie","GREEN": "Zvládnutá","MASTERED": "Mastered"}

return labels.get(status, status)

def status_class(status):classes = {"NEW": "status-new","RED": "status-red","YELLOW": "status-yellow","GREEN": "status-green","MASTERED": "status-mastered"}

return classes.get(status, "status-new")

def render_hero(subject_name, field_name, display_name):st.markdown(f"""Smart review{escape(subject_name)}{escape(field_name)}{escape(display_name)}Cieľ: {DAILY_GOAL}/deň""",unsafe_allow_html=True)

def render_question_header(q, p):status = p.get("status", "NEW")next_review = p.get("next_review") or "—"
...
def render_small_report_link(q, subject_name):form_url = (f"{REPORT_FORM_BASE_URL}"f"?usp=pp_url"f"&entry.424182118={q['id']}"f"&entry.1513577736={subject_name}")
...
def render_sidebar_card(title, rows):with st.sidebar.container(border=True):st.markdown(f"#### {title}")
...
def render_question_text_and_images(q):segments = re.split(r"(\S+.png|\S+.jpg)", q["text"])
...
st.sidebar.markdown(f"""{escape(APP_NAME)}{escape(APP_SUBTITLE)}""",unsafe_allow_html=True)

field_list = list(FIELDS.keys())f_idx = st.session_state.last_settings.get("field_idx", 0)
...
available_subjects = FIELDS[selected_field_name]subj_list = list(available_subjects.keys())default_subj = st.session_state.last_settings.get("subj_name")s_idx = subj_list.index(default_subj) if default_subj in subj_list else 0

st.session_state.selected_subject_name = st.sidebar.selectbox("Predmet",subj_list,index=s_idx)

selected_file = available_subjects[st.session_state.selected_subject_name]questions = load_questions(selected_file)

if not questions:render_hero(st.session_state.selected_subject_name,selected_field_name,st.session_state.display_name)
...
question_session_key = f"current_question_id_{selected_file}"nonce_key = f"answer_nonce_{selected_file}"

if nonce_key not in st.session_state:st.session_state[nonce_key] = 0

if question_session_key not in st.session_state:selected_question = choose_next_question(questions, current_data)st.session_state[question_session_key] = get_qid(selected_question)

q = get_question_by_id(questions, st.session_state[question_session_key])

if q is None:selected_question = choose_next_question(questions, current_data)st.session_state[question_session_key] = get_qid(selected_question)q = selected_question

qid = get_qid(q)q_progress = get_question_progress(current_data, qid)
...
render_hero(st.session_state.selected_subject_name,selected_field_name,st.session_state.display_name)

left_col, right_col = st.columns([0.70, 0.30], gap="large")

with left_col:with st.container(border=True):render_question_header(q, q_progress)render_question_text_and_images(q)
...
with right_col:counts = count_statuses(current_data, questions)daily_stats = get_daily_stats(current_data)new_limit = dynamic_daily_new_limit(counts)
...
counts = count_statuses(current_data, questions)daily_stats = get_daily_stats(current_data)new_limit = dynamic_daily_new_limit(counts)

answered_today = daily_stats.get("answered", 0)new_seen_today = daily_stats.get("new_seen", 0)

render_sidebar_card("Dnešný cieľ",[("Hlavný cieľ", f"{answered_today} / {DAILY_GOAL}"),("Zostáva", max(0, DAILY_GOAL - answered_today)),("Nové otázky", f"{new_seen_today} / {new_limit}"),("Problémové", counts.get("RED", 0))])

st.sidebar.divider()st.sidebar.markdown(f"""Prihlásený: {escape(st.session_state.display_name)}""",unsafe_allow_html=True)

if st.sidebar.button("Odhlásiť sa", use_container_width=True):logout_user()
