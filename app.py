...
def inject_css():
    st.markdown(
        ...
        <style>
            ...
            /* FINAL SPOTLIGHT SETUP PATCH */
            body.setup-active .stApp::after {
                content: "";
                position: fixed;
                inset: 0;
                background: rgba(2, 6, 23, 0.975) !important;
                backdrop-filter: grayscale(1) blur(12px) !important;
                z-index: 9990;
                pointer-events: none;
            }
            ...
            body.setup-active div[data-testid="stMetric"] {
                filter: grayscale(1) brightness(0.06) contrast(0.55) !important;
...
def render_center_date_setup(selected_field_name, current_exam_date):
    ...
    st.markdown(
        f"""
        <div class="center-calendar-card">
            <div class="center-calendar-step">Krok 2/3</div>
            <div class="center-calendar-title">Zadaj termín skúšky</div>
            <div class="center-calendar-text">
    ...
    selected_exam_date = st.date_input(
        ...
        format="DD.MM.YYYY",
        key=f"center_setup_exam_date_{sanitize_key(selected_field_name)}"
    )

    if st.button("Potvrdiť termín skúšky", use_container_width=True, key="confirm_center_exam_date"):
...
if setup_active and step == 1:
    setup_sidebar_note(1, "Vyber si odbor", "Vyber odbor, na ktorý sa pripravuješ.")
